        ...
        >
          <div className="flex flex-wrap gap-4 md:gap-6">
            <div className="flex flex-col">
              <span className="text-xs text-slate-600 font-bold uppercase tracking-[0.2em] mb-0.5">Rank</span>
              <div className="flex items-center gap-1 h-[14px]">
                <span className={`text-xs font-mono tabular-nums leading-none ${isCompleted ? 'text-slate-600' : 'text-amber-500'}`}>{'★'.repeat(Math.max(1, Math.ceil(goal.difficulty / 2)))}</span>
              </div>
            </div>
            <div className="flex flex-col">
            ...
            <div className="flex flex-col">
              ...
              <span className={`text-xs font-mono tabular-nums ${isCompleted ? 'text-slate-600' : 'text-purple-400'}`}>+{(goal.reward_alpha * 100).toFixed(0)}%</span>
            </div>
          </div>
