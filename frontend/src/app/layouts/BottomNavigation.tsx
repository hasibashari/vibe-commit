    ...
    <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-md z-[100] pb-[env(safe-area-inset-bottom)]">
      <div className="flex justify-around items-center h-16 px-1 bg-slate-950/90 backdrop-blur-2xl border border-slate-700/50 shadow-[0_10px_40px_rgba(0,0,0,0.8),inset_0_1px_rgba(255,255,255,0.1)] rounded-2xl relative">
        ...
        <button onClick={() => setActiveTab('character')} className="flex flex-col items-center justify-center w-[18%] h-full gap-1 group relative">
          ...
          <Shield className={`w-[22px] h-[22px] transition-all duration-300 ${activeTab === 'character' ? 'text-rose-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.8)] -translate-y-0.5' : 'text-slate-500'}`} />
          <span className={`text-[8px] font-bold uppercase tracking-widest transition-all duration-300 absolute bottom-1.5 ${activeTab === 'character' ? 'opacity-100 text-rose-300' : 'opacity-0 translate-y-1'}`}>Status</span>
        </button>
        ...
        <button onClick={() => setActiveTab('dashboard')} className="flex flex-col items-center justify-center w-[18%] h-full gap-1 group relative">
          ...
          <BarChart3 className={`w-[22px] h-[22px] transition-all duration-300 ${activeTab === 'dashboard' ? 'text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)] -translate-y-0.5' : 'text-slate-500'}`} />
          <span className={`text-[8px] font-bold uppercase tracking-widest transition-all duration-300 absolute bottom-1.5 ${activeTab === 'dashboard' ? 'opacity-100 text-cyan-300' : 'opacity-0 translate-y-1'}`}>Hub</span>
        </button>
          ...
          <div className="absolute inset-x-0 -top-10 flex justify-center">
            <div className="relative">
              ...
              <button 
                ...
                className="relative w-14 h-14 bg-gradient-to-br from-cyan-400 to-emerald-500 rounded-full flex flex-col items-center justify-center text-slate-950 shadow-[0_4px_20px_rgba(6,182,212,0.5)] active:scale-95 transition-transform border-[3px] border-slate-900 overflow-hidden group"
              >
        ...
        <button onClick={() => setActiveTab('quests')} className="flex flex-col items-center justify-center w-[18%] h-full gap-1 group relative">
          ...
          <Target className={`w-[22px] h-[22px] transition-all duration-300 ${activeTab === 'quests' ? 'text-indigo-400 drop-shadow-[0_0_8px_rgba(129,140,248,0.8)] -translate-y-0.5' : 'text-slate-500'}`} />
          <span className={`text-[8px] font-bold uppercase tracking-widest transition-all duration-300 absolute bottom-1.5 ${activeTab === 'quests' ? 'opacity-100 text-indigo-300' : 'opacity-0 translate-y-1'}`}>Quest</span>
        </button>

        <button onClick={onNewQuest} className="flex flex-col items-center justify-center w-[18%] h-full gap-1 group relative">
          ...
          <Plus className="w-[22px] h-[22px] transition-all duration-300 text-slate-500 hover:text-purple-400 active:scale-90 active:text-purple-400 group-hover:-translate-y-0.5" />
          <span className="text-[8px] font-bold uppercase tracking-widest text-slate-500 transition-all duration-300 absolute bottom-1.5 opacity-0 group-hover:opacity-100 group-hover:text-purple-300">New</span>
        </button>
