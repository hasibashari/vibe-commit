...
export class LogController {
  ...
  static async createLog(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Consolidation 1: Fetch user_id and goal details in a single query
      const goalRes = await db.query(
        'SELECT user_id, difficulty, reward_alpha, type, status FROM goals WHERE id = $1', 
        [goalId]
      );
      const goal = goalRes.rows[0] as any;
      if (!goal) {
      ...
      try {
        ...
        // Consolidation 2: Fetch user stats including sandbox_date_offset in a single query
        const userRes = await client.query(
          'SELECT id, hp, mana, level, exp, last_penalty_date, sandbox_date_offset FROM users WHERE id = $1', 
          [goal.user_id]
        );
        ...
        const offset = user.sandbox_date_offset || 0;
        ...
        if (goal.type === 'one-off' && goal.status === 'completed') {
          await client.query('ROLLBACK');
          res.status(400).json({ error: 'Quest ini sudah diselesaikan sebelumnya.' });
          return;
        }
        ...
        if (goal.type === 'one-off') {
          ...
          expGain = Math.floor(10 * (goal.difficulty || 1.0) * gachaMultiplier);
          ...
          expGain = Math.floor(10 * (goal.difficulty || 1.0) * (goal.reward_alpha || 0.5));
        }
